...
//		System.out.println(voList.size());
	...
	public SverResponse<OrderVo> generateOrder(Integer userid, Integer addrId) {
		...
		//6.减少库存
		for(OrderItem orderItem:orderItems) {
			Product Product = ProductDao.findProductById(orderItem.getGoodsId());
			//减少数量
...
//			}
//			return SverResponse.createByErrorMessage("出现错误！");
		}
		//7.请空购物车
		ProductDao.deleteCartProduct(userid);
//		int ro = ProductDao.deleteCartProduct(userid);
//		if (ro>0) {
//			SverResponse.createRespBySuccessMessage("清空购物车成功");
//		}
		//8.封装返回前端
